    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <span>{{ examPaper?.title }} - 自动组卷</span>
        </div>
      </template>
      ...
      <div v-if="examPaper" class="exam-paper-info">
        <el-descriptions :column="3" border size="small">
          ...
          <el-descriptions-item label="考试分类">{{ getCategoryName(examPaper.examCategory) }}</el-descriptions-item>
          <el-descriptions-item label="考试时长">{{ examPaper.duration }}分钟</el-descriptions-item>
          <el-descriptions-item label="允许重复考试">{{ examPaper.allowRetake ? '是' : '否' }}</el-descriptions-item>
        </el-descriptions>
      </div>
...
export default {
  ...
  setup() {
    ...
    const loadAutoRules = async () => {
      try {
        const response = await api.get(`/exam-papers/${route.params.id}/auto-rules`)
        if (response.data.success) {
    ...
    const submitRule = async () => {
      try {
        ...
        if (isEditRule.value) {
          await api.put(`/exam-papers/${route.params.id}/auto-rules/${ruleForm.id}`, ruleForm)
          ElMessage.success('更新规则成功')
        } else {
          await api.post(`/exam-papers/${route.params.id}/auto-rules`, ruleForm)
    ...
    const removeRule = async (rule) => {
      try {
        ...
        await api.delete(`/exam-papers/${route.params.id}/auto-rules/${rule.id}`)
    ...
    const autoGenerate = async () => {
      try {
        ...
        const response = await api.post(`/exam-papers/${route.params.id}/auto-generate`)
