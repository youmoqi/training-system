  ...
  <div class="page-container">
    <el-card>
      ...
      <div class="questions-section">
          ...
          <el-table-column label="操作" width="340" fixed="right" align="center">
            <template #default="scope">
              ...
              <el-button size="small" type="info" @click="moveQuestionDown(scope.row)" :icon="ArrowDown"
                         :disabled="scope.$index === (exam?.questions?.length - 1)">
                下移
              </el-button>
        ...
        <div v-if="!exam?.questions || exam.questions.length === 0" class="no-questions">
    ...
    <el-dialog v-model="addQuestionDialogVisible" title="添加题目" width="1000px">
      <div class="add-question-container">
        <!-- 分值设置提示 -->
        <div v-if="exam" class="score-settings-info">
          <el-alert
              title="试卷分值设置"
              type="info"
              :closable="false"
          ...
          >
            <template #default>
              <div class="score-settings">
                <span>单选题：{{ exam.singleChoiceScore || 2 }}分</span>
                <span>多选题：{{ exam.multipleChoiceScore || 4 }}分</span>
                <span>判断题：{{ exam.trueFalseScore || 2 }}分</span>
                <span>填空题：{{ exam.fillBlankScore || 3 }}分</span>
                <span>简答题：{{ exam.shortAnswerScore || 5 }}分</span>
              </div>
            </template>
          </el-alert>
        </div>
...
export default {
  ...
  setup() {
    ...
    const addSelectedQuestions = async () => {
      try {
        ...
        const promises = selectedQuestions.value.map((question, index) => {
          const order = (exam.value.questions?.length || 0) + index + 1
    ...
    const updateQuestionScore = async () => {
      try {
        await api.put(`/exams/${route.params.id}/questions/${scoreForm.questionId}/score`, null, {
    ...
    const removeQuestion = async (question) => {
      try {
        ...
        await api.delete(`/exams/${route.params.id}/questions/${question.questionId}`)
    ...
    const moveQuestionUp = async (question) => {
      ...
      try {
        ...
        await api.put(`/exams/${route.params.id}/questions/order`, questionIds)
