    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <span>试卷详情</span>
          <div>
            <el-button type="success" @click="manageQuestions" v-if="exam">
              管理题目
            </el-button>
          </div>
        </div>
      ...
      <div v-if="exam" class="exam-paper-info">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="可见角色" :span="2">
            <el-tag v-for="role in exam.visibleRoles" :key="role" style="margin-right: 5px">
              {{ role?.role.name }}
            </el-tag>
          </el-descriptions-item>
      ...
      <!-- 题目列表 -->
      <div v-if="exam && exam.questions && exam.questions.length > 0" class="questions-section">
        <h3>题目列表</h3>
        <el-table :data="exam.questions" style="width: 100%">
      ...
      <div v-else-if="exam" class="no-questions">
...
export default {
  ...
  setup() {
    ...
    const manageQuestions = () => {
      router.push(`/admin/exams/${route.params.id}/questions`)
    }
