...
export default {
  ...
  setup() {
    ...
    const loadQuestions = async () => {
      try {
        const response = await api.get(`/questions/question-bank/${questionBankId.value}`)
    ...
    const removeOption = (index) => {
      ...
      if (Array.isArray(formData.answers)) {
        formData.answers = formData.answers.filter(answer => 
          formData.options.includes(answer)
        )
      }
    }
    ...
    const saveQuestion = async () => {
      try {
        ...
        const questionData = {
          ...
          options: isChoiceQuestion.value ? formData.options : null,
          answers: formData.type === 'SUBJECTIVE' ? [formData.answers] : (Array.isArray(formData.answers) ? formData.answers : [formData.answers]),
