      ...
      <div class="card-body">
        ...
        <div v-if="!loading && examHistories.length === 0" class="empty-state">
          <el-empty description="暂无考试历史记录">
            <el-button type="primary" @click="$router.push('/dashboard/exams')">
              去购买试卷
            </el-button>
          </el-empty>
        </div>
  ...
  setup() {
    ...
    const loadExamHistories = async () => {
      ...
      try {
        ...
        const response = await api.get(`/exam-results/user/${userId}`, {params})
          ...
          for (let history of examHistories.value) {
            try {
              ...
              if (examResponse.data.success) {
                history.exam = examResponse.data.data
              }
            } catch (error) {
    ...
    const viewExamResult = (history) => {
      router.push(`/dashboard/exams/${history.examId}/results/${history.id}`)
    }

    const retakeExam = async (history) => {
      try {
        // 检查是否可以重新考试
        const canRetakeResponse = await api.get(`/exams/${history.examId}/can-retake/${store.getters.userId}`)
        if (canRetakeResponse.data.success && canRetakeResponse.data.data) {
          router.push(`/dashboard/exams/${history.examId}/exam`)
        } else {
    ...
    const canRetake = (history) => {
      ...
      if (history.exam) {
        return history.exam.allowRetake && history.attemptNumber < history.exam.maxAttempts
      }
      return false
    }
