  ...
  setup() {
    ...
    const loadExam = async () => {
      try {
        ...
        // 检查是否可以参加考试
        const canRetakeResponse = await api.get(`/exams/${examId}/can-retake/${store.getters.userId}`)
        ...
        // 设置时间限制
        timeLimit.value = exam.value.duration || 120
    ...
    const confirmSubmit = async () => {
      try {
        ...
        // 提交考试
        const response = await api.post('/exams/submit', submitData)
