      ...
      <div class="purchase-container">
        ...
        >
          ...
          <el-table-column label="操作" width="120">
            <template #default="scope">
              <el-button size="small" type="primary" @click="purchaseExam(scope.row)">
                购买
              </el-button>
            </template>
          </el-table-column>
        ...
        <!-- 空状态 -->
        <el-empty v-if="!purchaseLoading && availableExams.length === 0" description="暂无可购买试卷"/>
      </div>
  ...
  setup() {
    ...
    const loadExams = async () => {
      ...
      try {
        ...
        const response = await api.get('/exams/available/page', {params})
    ...
    const loadAvailableExams = async () => {
      ...
      try {
        ...
        const response = await api.get('/exams/purchasable/page', {params})
        if (response.data.success) {
          availableExams.value = response.data.data.content
    ...
    const purchaseExam = async (paper) => {
      try {
        ...
        await api.post(`/exams/${paper.id}/purchase`)
        ElMessage.success('购买成功')
        purchaseDialogVisible.value = false
        loadExams() // 重新加载已购买的试卷
      } catch (error) {
        if (error !== 'cancel') {
    ...
    const viewExam = (paper) => {
      router.push(`/dashboard/exams/${paper.id}`)
    }

    const takeExam = (paper) => {
      router.push(`/dashboard/exams/${paper.id}/exam`)
    }
